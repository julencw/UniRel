...
class UniRelDataProcessor(object):
    def __init__(
        ...
        if dataset_name == "nyt":
            self.pred2text = (
                dataprocess.rel2text.nyt_rel2text
            )  # TODO: dictionary for DialogRE
            ...
            for pred in self.label2id.keys():
                try:
                    self.pred2text[pred] = dataprocess.rel2text.webnlg_rel2text[
                        pred
                    ]
                except KeyError:
                    print(pred)
        ...
        self.pred2idx = {}  # mapping from predicate to index
        idx = 0
        self.pred_str = ""
        for k in self.pred2text:
            self.pred2idx[k] = idx
            ...
            idx += 1
        self.pred_str = self.pred_str[:-1]  # removes the last space
    ...
    def _pre_process(self, path, token_len, is_predict, data_nums):
        ...
        for line in tqdm(data):
            ...
            if token_encode_len > token_len + 2:
                continue
            spo_list = set()  # (subject, predicate, object) set
            spo_span_list = set()  # (subject_span, predicate, object_span) set
            # [CLS] texts [SEP] rels
            ...
            e2e_set = set()  # entity to entity set
            h2r_dict = dict()  # head to relation dict (head = subject)
            t2r_dict = dict()  # tail to relation dict (tail = object)
            spo_tail_set = set()
            ...
            for spo in line["relation_list"]:
                ...
                pred_idx = self.pred2idx[pred]  # predicate index
                plus_token_pred_idx = (
                    pred_idx + token_len + 2
                )  # predicate index + token_len + 2
                spo_span_list.add((tuple(sub_span), pred_idx, tuple(obj_span)))

                h_s, h_e = sub_span
                t_s, t_e = obj_span
                # Entity-Entity Interaction
                head_matrix[h_s + 1][t_s + 1] = 1  # head start to tail start
                head_matrix[t_s + 1][h_s + 1] = 1  # tail start to head start
                tail_matrix[h_e][t_e] = 1  # head end to tail end
                tail_matrix[t_e][h_e] = 1  # tail end to head end
                span_matrix[h_s + 1][h_e] = 1  # head start to head end
                span_matrix[h_e][h_s + 1] = 1  # head end to head start
                span_matrix[t_s + 1][t_e] = 1  # tail start to tail end
                span_matrix[t_e][t_s + 1] = 1  # tail end to tail start
